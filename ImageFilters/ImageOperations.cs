...
namespace ImageFilters
{
    public class ImageOperations
    {
        ...
        public static byte[,] OpenImage(string ImagePath)
        {
            ...
            int wres = Width  - 2 * r;
            int hres = Height - 2 * r;
            Bitmap res_img = new Bitmap(wres, hres);
            BitmapData res_res = res_img.LockBits(new Rectangle(0, 0, wres, hres), ImageLockMode.WriteOnly, PixelFormat.Format24bppRgb);
            int res_byte = res_res.Stride * res_res.Height;
            byte[] res = new byte[res_byte];
            return Buffer;
        }
